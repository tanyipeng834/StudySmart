<template>
     <div>
          <div class="d-flex justify-content-end mt-4">
               <button class="btn" v-if="addTest===false" @click="addTest=!addTest"><i
                         class="fa-regular fa-pen-to-square fa-lg text-right"></i>Add Test</button>
               <div v-else>
                    <button class="btn" @click="addTest=!addTest"><i
                              class="fa-solid fa-xmark  fa-lg text-right"></i>Cancel</button>
                    <button class="btn" @click="onSubmit()"><i
                              class="fa-solid fa-check fa-lg text-right"></i>Submit</button>
          ...
          <ul class="list-group">
               ...
               <li :class="[addTest ? 'list-group-item' : 'hide']" @keyup.enter="onSubmit()">
                    ...
                    <div class="col-4 ms-1">
                         <input v-model="date" type="text" class="form-control form-control-sm"
                              placeholder="mm/dd/yyyy">
                    </div>


               </li>
               <li v-for="test in tests" class="list-group-item d-flex justify-content-around align-items-start">

                    <div class="ms-2 me-auto col-7">
                         <div class="fw-bold">{{test.testName}}</div>
                         <p><small>{{test.subject}} . {{test.date}}</small></p>
                    </div>

                    <div class="col-4 d-flex justify-content-center">
                         <span class="p-2 badge bg-primary rounded-pill mx-auto">{{daysLeft(test.date)}}</span>
                    </div>

                    <div class="col-1 ">
                         <button type="button" class="btn position-absolute top-0 end-2"
                              v-on:click="deleteTest(test.id)"><i v-on:click="deleteTest($event)"
